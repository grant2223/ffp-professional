...
// ── OVERVIEW: the strength of your business, from real numbers (pro_grow_overview) ──
function _aed(n){ return 'AED ' + (Number(n) || 0).toLocaleString(); }
function _growMetric(big, label, sub){
  return '<div style="flex:1;min-width:0;background:var(--ffp-bg-card);border:1px solid var(--ffp-border);border-radius:14px;padding:14px 12px;">'
    + '<div style="font-size:21px;font-weight:900;color:var(--ffp-text);line-height:1;">' + big + '</div>'
    + '<div class="psub" style="margin:5px 0 0;text-transform:uppercase;font-size:9px;font-weight:800;letter-spacing:.5px;">' + label + '</div>'
    + (sub ? '<div style="font-size:10.5px;color:var(--ffp-text-dim);font-weight:700;margin-top:2px;">' + sub + '</div>' : '') + '</div>';
}
function _growNowTab(){
  var o = _growOverview || {};
  var focus = o.focus || 'Reading your business…';
  var hero = '<div style="position:relative;background:linear-gradient(135deg,#0a3e44 0%,#0e5a63 60%,#127d78 100%);border-radius:20px;padding:18px;color:#fff;margin-bottom:16px;overflow:hidden;box-shadow:0 12px 30px rgba(10,62,68,.28);">'
    ...
    + '<div style="position:relative;"><div style="font-size:10px;font-weight:800;letter-spacing:1.6px;color:#8fe0d0;">YOUR FOCUS RIGHT NOW</div>'
    + '<div style="font-size:16px;font-weight:800;line-height:1.35;margin-top:8px;">' + _growEsc(focus) + '</div></div></div>';
  var ret = (o.retention_pct == null ? '—' : o.retention_pct + '%');
  var roww = function (a, b){ return '<div style="display:flex;gap:10px;margin-bottom:10px;">' + a + b + '</div>'; };
  var grid = roww(_growMetric(o.active_clients || 0, 'Active clients', (o.new_30d || 0) + ' new · 30d'), _growMetric(_aed(o.revenue_30d), 'This month', _aed(o.revenue_all) + ' all'))
    + roww(_growMetric(_aed(o.ltv), 'Avg / client', (o.paying_clients || 0) + ' paying'), _growMetric(ret, 'Kept 90d+', (o.archived ? o.archived + ' left' : '')))
    + roww(_growMetric(o.sessions || 0, 'Sessions', ''), _growMetric(_aed(o.outstanding), 'Unpaid', (o.outstanding > 0 ? 'chase it' : 'all in')));
  var cols = ['#0a3e44', '#2ba8e0', '#FFCC00', '#869599'];
  var mix = '';
  if (o.methods && o.methods.length){
    mix = '<div class="form-section-title" style="margin-top:6px;">How you get paid</div><div style="display:flex;height:12px;border-radius:6px;overflow:hidden;margin-bottom:8px;">'
      + o.methods.map(function (m, i){ return '<div style="width:' + (m.pct || 0) + '%;background:' + cols[i % 4] + ';"></div>'; }).join('')
      + '</div><div style="display:flex;flex-wrap:wrap;gap:12px;margin-bottom:16px;">'
      + o.methods.map(function (m, i){ return '<div style="font-size:11px;font-weight:700;color:var(--ffp-text-muted);display:flex;align-items:center;gap:5px;"><span style="width:9px;height:9px;border-radius:50%;background:' + cols[i % 4] + ';"></span>' + _growEsc(m.method) + ' ' + (m.pct || 0) + '%</div>'; }).join('') + '</div>';
  }
  var top = '';
  if (o.top_client && o.top_client.name){
    top = '<div style="display:flex;align-items:center;gap:11px;background:var(--ffp-bg-card);border:1px solid var(--ffp-border);border-radius:14px;padding:13px 15px;margin-bottom:12px;"><span class="ms" style="color:#FFCC00;">star</span><div style="flex:1;min-width:0;"><div style="font-size:9.5px;font-weight:800;letter-spacing:.7px;text-transform:uppercase;color:var(--ffp-text-dim);">Most valuable client</div><div style="font-size:14px;font-weight:800;color:var(--ffp-text);">' + _growEsc(o.top_client.name) + '</div></div><div style="font-size:14px;font-weight:900;color:#0a3e44;">' + _aed(o.top_client.amount) + '</div></div>';
  }
  var pct = Math.round((o.steps_done || 0) * 100 / (o.steps_total || 8));
  var prog = '<div style="background:var(--ffp-bg-card);border:1px solid var(--ffp-border);border-radius:14px;padding:14px 15px;margin-bottom:16px;cursor:pointer;" onclick="growTab(\'road\')"><div style="display:flex;justify-content:space-between;margin-bottom:8px;"><div style="font-size:12.5px;font-weight:800;color:var(--ffp-text);">Road map progress</div><div style="font-size:12px;font-weight:800;color:#0a3e44;">' + (o.steps_done || 0) + ' / ' + (o.steps_total || 8) + '</div></div><div style="height:8px;border-radius:4px;background:#edf1f2;overflow:hidden;"><div style="width:' + pct + '%;height:100%;background:linear-gradient(90deg,#0a3e44,#2ba8e0);"></div></div></div>';
  var coach = '<button type="button" onclick="growTalkPulse()" style="width:100%;background:linear-gradient(135deg,#0a3e44,#2ba8e0);color:#fff;border:none;border-radius:13px;padding:14px;font-size:14.5px;font-weight:800;font-family:inherit;cursor:pointer;box-shadow:0 8px 18px rgba(43,168,224,.30);display:flex;align-items:center;justify-content:center;gap:8px;"><span class="ms">sports</span> Work through it with your Coach</button>';
  return hero + grid + mix + top + prog + coach;
}
function growTalkPulse(){
  var o = _growOverview || {};
  var seed = 'Give me a straight diagnostic of my coaching business and hold me accountable. Right now: ' + (o.active_clients||0) + ' active clients (' + (o.new_30d||0) + ' new in 30 days), AED ' + (o.revenue_all||0) + ' collected all-time (AED ' + (o.revenue_30d||0) + ' this month), AED ' + (o.outstanding||0) + ' unpaid, about AED ' + (o.ltv||0) + ' per client, ' + (o.sessions||0) + ' sessions logged, ' + (o.posts_7d||0) + ' of my own training posts this week, ' + (o.steps_done||0) + ' of ' + (o.steps_total||8) + ' road-map steps done. My focus looks like: ' + (o.focus||'') + ' Ask me sharp questions, tell me the truth, and give me my single next move.';
...
async function renderGrow(){
  ...
  var st = {}, ov = {};
  try { var ro = await window.supabase.rpc('pro_grow_overview', { p_pro: pid }); ov = (ro && ro.data) || {}; } catch (e) { console.error('[FFP Grow] overview', e); }
  try { var r = await window.supabase.rpc('pro_grow_state', { p_pro: pid }); st = (r && r.data) || {}; } catch (e) { console.error('[FFP Grow] state', e); }
  _growOverview = ov; _growState = st;
  _stepDone = {};
...
async function _growRefreshPulse(){
  var pid = _growProvId(); if (!pid) return;
  try { var ro = await window.supabase.rpc('pro_grow_overview', { p_pro: pid }); if (ro && ro.data) { _growOverview = ro.data; if (_growTab === 'now' && document.getElementById('grow-body')) _growPaint(); } } catch (e) {}
}
